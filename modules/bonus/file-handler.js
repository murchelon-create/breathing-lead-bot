...
class FileHandler {
  ...
  // Цель: перевести тёплого лида в первую оплату (пробный видеоурок 1 500 ₽).
  // Кнопка — callback, бот перехватывает нажатие и запускает мини-диалог сбора телефона.
  async showPostPDFMenu(ctx) {
    const message =
      `🎯 *ХОТИТЕ УЗНАТЬ МЕТОД БУТЕЙКО ГЛУБЖЕ?*\n\n` +
      `Получите пробный видеоурок — 40 минут:\n\n` +
      `Что вы узнаете:\n` +
      `• Как измерить свою контрольную паузу\n` +
      `• Почему глубокое дыхание вредит здоровью\n` +
      `• Первые техники для снятия симптомов\n` +
      `• Домашнее задание с контролем выполнения\n\n` +
      `✅ Поддержка в Telegram после урока\n` +
      `✅ Контроль выполнения\n\n` +
      `Стоимость — 1 500 ₽\n\n` +
    ...
    const keyboard = [
      [Markup.button.callback('👉  Получить пробный видеоурок  —  1 500 ₽', 'book_trial')]
    ];
  ...
  async handleBookTrial(ctx) {
    ...
    await ctx.reply(
      ...
      `📞 Напишите ваш номер телефона — я свяжусь с вами в течение 2 часов и пришлю ссылку на видеоурок:\n\n` +
  ...
  async handleTrialPhoneInput(ctx, notifyAdmin) {
    ...
    console.log(`📞 Получен телефон для пробного видеоурока: ${phone}, пользователь: ${userInfo.telegram_id}`);
    ...
    try {
      ...
      console.log('✅ Заявка на пробный видеоурок записана');
    } catch (err) {
    ...
    await ctx.reply(
      ...
      `📞 Телефон: ${phone}\n\n` +
      `Я свяжусь с вами в течение 2 часов и пришлю ссылку на пробный видеоурок.\n\n` +
      `_Стоимость: 1 500 ₽ — оплата перед отправкой ссылки._`,
      { parse_mode: 'Markdown' }
    );
  }
